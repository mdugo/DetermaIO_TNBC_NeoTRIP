...
inputPath<-"~/Desktop/hsr_bioinfo/projects/NeoTRIP/rnaseq/posters_papers/manuscript_baseline_IOscore/second_rebuttal/github/data"
...
#*********************************************************************************
# Supplementary Figure 5 - Comparison between IO score and other immune signatures
#*********************************************************************************

# correlation between IO score and other signatures

df2<-df[!is.na(df$IO_cat_qPCR),]

correl<-data.frame(Signature=colnames(df)[c(15:25)],Cohort="RNA-Seq",Pearson=0)
for(i in 1:nrow(correl)){
  correl$Pearson[i]<-cor(df$IO_cont_NGS, df[,colnames(df)==correl$Signature[i]], method="pearson")
}
correl2<-data.frame(Signature=colnames(df)[c(15:25)],Cohort="RT-qPCR",Pearson=0)
for(i in 1:nrow(correl2)){
  correl2$Pearson[i]<-cor(df$IO_cont_qPCR, df[,colnames(df)==correl2$Signature[i]], method="pearson", use="pairwise.complete.obs")
}
correl<-rbind(correl2, correl)
correl$Cohort<-factor(correl$Cohort, levels=c("RT-qPCR","RNA-Seq"))
correl<-correl[order(correl$Cohort,correl$Pearson),]
correl$Signature<-gsub("G6 ","G6-",gsub("T cell","T-cell",gsub("_"," ",correl$Signature)))
correl$Signature<-factor(correl$Signature,levels = unique(correl$Signature))
g1<-ggplot(data=correl, aes(x=Signature, y=Pearson, fill=Cohort)) +
  geom_hline(yintercept = seq(0,1,0.25), linetype="dashed",col="grey80") +
...
# forest plot RT-qPCR cohort

variables<-colnames(df)[c(15:25)]
res<-data.frame(Cohort="RT-qPCR cohort (N = 220)", Variable=rep(c("IO score", variables), each=2),
                Arm=rep(c("CT/A","CT"),12),
                OR=0, CI2.5=0, CI97.5=0, p=0, p.int=0)
...
res$OR[res$Variable=="IO score"]<-c(exp(coef(fit.cta)[2]), exp(coef(fit.ct)[2]))
res$CI2.5[res$Variable=="IO score"]<-c(exp(confint(fit.cta)[2,1]), exp(confint(fit.ct)[2,1]))
res$CI97.5[res$Variable=="IO score"]<-c(exp(confint(fit.cta)[2,2]), exp(confint(fit.ct)[2,2]))
res$p[res$Variable=="IO score"]<-c(summary(fit.cta)$coefficients[2,4], summary(fit.ct)$coefficients[2,4])
res$p.int[res$Variable=="IO score"]<-rep(summary(fit.interact)$coefficients[4,4], 2)

for(i in 1:length(variables)){
  myformula<-paste("pCR.num ~", variables[i])
  myformula.int<-paste("pCR.num ~", variables[i],"* Arm")
  fit.cta<-glm(formula = myformula, family =binomial, data=df2[df2$Arm=="CT/A",])
  fit.ct<-glm(formula = myformula, family =binomial, data=df2[df2$Arm=="CT",])
  fit.interact<-glm(formula = myformula.int, family =binomial, data=df2)
  res$OR[res$Variable==variables[i]]<-c(exp(coef(fit.cta)[2]), exp(coef(fit.ct)[2]))
  res$CI2.5[res$Variable==variables[i]]<-c(exp(confint(fit.cta)[2,1]), exp(confint(fit.ct)[2,1]))
  res$CI97.5[res$Variable==variables[i]]<-c(exp(confint(fit.cta)[2,2]), exp(confint(fit.ct)[2,2]))
  res$p[res$Variable==variables[i]]<-c(summary(fit.cta)$coefficients[2,4], summary(fit.ct)$coefficients[2,4])
  res$p.int[res$Variable==variables[i]]<-rep(summary(fit.interact)$coefficients[4,4], 2)
}

# forest plot RNA-Seq cohort

res2<-data.frame(Cohort="RNA-Seq cohort (N = 242)", Variable=rep(c("IO score", variables), each=2),
                 Arm=rep(c("CT/A","CT"),12),
                 OR=0, CI2.5=0, CI97.5=0, p=0, p.int=0)
...
res2$OR[res2$Variable=="IO score"]<-c(exp(coef(fit.cta)[2]), exp(coef(fit.ct)[2]))
res2$CI2.5[res2$Variable=="IO score"]<-c(exp(confint(fit.cta)[2,1]), exp(confint(fit.ct)[2,1]))
res2$CI97.5[res2$Variable=="IO score"]<-c(exp(confint(fit.cta)[2,2]), exp(confint(fit.ct)[2,2]))
res2$p[res2$Variable=="IO score"]<-c(summary(fit.cta)$coefficients[2,4], summary(fit.ct)$coefficients[2,4])
res2$p.int[res2$Variable=="IO score"]<-rep(summary(fit.interact)$coefficients[4,4], 2)

for(i in 1:length(variables)){
  myformula<-paste("pCR.num ~", variables[i])
  myformula.int<-paste("pCR.num ~", variables[i],"* Arm")
  fit.cta<-glm(formula = myformula, family =binomial, data=df[df$Arm=="CT/A",])
  fit.ct<-glm(formula = myformula, family =binomial, data=df[df$Arm=="CT",])
  fit.interact<-glm(formula = myformula.int, family =binomial, data=df)
  res2$OR[res2$Variable==variables[i]]<-c(exp(coef(fit.cta)[2]), exp(coef(fit.ct)[2]))
  res2$CI2.5[res2$Variable==variables[i]]<-c(exp(confint(fit.cta)[2,1]), exp(confint(fit.ct)[2,1]))
  res2$CI97.5[res2$Variable==variables[i]]<-c(exp(confint(fit.cta)[2,2]), exp(confint(fit.ct)[2,2]))
  res2$p[res2$Variable==variables[i]]<-c(summary(fit.cta)$coefficients[2,4], summary(fit.ct)$coefficients[2,4])
  res2$p.int[res2$Variable==variables[i]]<-rep(summary(fit.interact)$coefficients[4,4], 2)
}

res<-rbind(res, res2)
res$Cohort<-factor(res$Cohort, levels=c("RT-qPCR cohort (N = 220)","RNA-Seq cohort (N = 242)"))
res$Arm<-factor(res$Arm, levels=c("CT/A","CT"))
res<-res[order(res$Cohort,res$Arm,res$OR),]
res$Variable<-gsub("G6 ","G6-",gsub("T cell","T-cell",gsub("_"," ",res$Variable)))
res$Variable<-factor(res$Variable, levels=unique(res$Variable))
res$p.int<-round(res$p.int, 3)
res$Significance<-factor(ifelse(res$p<0.05,"p < 0.05","n.s."), levels=c("p < 0.05","n.s."))
res$Significance<-ifelse(res$p<0.05,"*","")

pFor<-ggplot(data=res[res$Arm!="CT/A+CT",], aes(x=Variable, y=log(OR), color=Arm)) +
  geom_pointrange(aes(ymin=log(CI2.5), ymax=log(CI97.5)), size=0.7, position = position_dodge(width = 1)) +
  geom_text(aes(x=Variable, y=log(OR),label=Significance, group=Arm),position = position_dodge(width = 1), color="black", size=6, vjust=0.76, hjust=0.47, fontface="bold") +
  coord_flip() +
  scale_y_continuous(breaks=seq(-1,3,1), limits = c(-1,3)) +
  scale_color_manual(values=carto_pal(12,"Bold")[8:7]) +
  theme_pubr(border=T, legend="right") +
  geom_hline(yintercept = 0, linetype="dashed") +
  xlab("") +
  geom_text(data=unique(res[res$Arm!="CT/A+CT",c(1,2,8)]), aes(x=Variable, y=2.7, label=p.int), inherit.aes = F) +
  annotate(geom="text", x= 13, y=2.7, 
           label="italic(P)[interaction]", 
           parse=TRUE, 
           hjust=0.5, size=4) +
  facet_wrap(~Cohort, ncol=1) +
  expand_limits(x= c(0, 13.7))

pdf(paste0(outputPath, "/Supplementary_Figure_5.pdf"),width=12,height=6.4)
cowplot::plot_grid(g1, pFor, labels=c("A","B"), label_size = 22, rel_widths = c(0.81,1))
dev.off()


#***********************************************************************
# Supplementary Figure 6 - Comparison of TNBC subtypes classifications
#***********************************************************************

# sankey plot for comparison of TNBC subtypes classification

df$TNBC6<-factor(df$TNBC6, levels=c("LAR","BL1","BL2","M","MSL","IM","ND"))
df2<-df[,c("TNBCtypes", "TNBC6")]

df2 <- df2 %>%
  make_long(colnames(df2))
factor.levels<-union(levels(factor(df2$node)), levels(factor(df2$next_node)))
df2$node<-factor(df2$node,levels=factor.levels)
df2$next_node<-factor(df2$next_node,levels=factor.levels)
stages<-levels(df2$x)
count.stages<-vector("list", length = length(stages))
names(count.stages)<-stages
for(i in 1:length(count.stages)){
  count.stages[[i]]<-table(df2$node[df2$x%in%stages[i]])
}
...
for(i in 1:length(lab.pos.stages)){
  ...
  for(j in 2:length(lab.pos.stages[[i]])){
    lab.pos.stages[[i]][j]<-(lab.pos.stages[[i]][j] - (count.stages[[i]][j]/2)) + (10*(j-1))
  }
}
...
for(i in 1:length(stages)){
  tnxn[,i]<-as.vector(table(df2$node[df2$x==stages[i]]))
}
...
p<-ggplot(data=df2, aes(x = x,
                        ...
                        next_node = next_node,
                        fill=node)) +
  geom_sankey(smooth=8,type="alluvial",
              node.color = "black",
              alpha=0.5,
              space=10,
              width=0.2,
  ...
  scale_x_discrete(labels=c("TNBC subtypes\n(Ring et al.)","TNBC subtypes\n(Lehmann et al.)")) +
  scale_fill_manual(name="",values=c(pal_npg("nrc")(6)[1:2],"gold",pal_npg("nrc")(6)[3:5],"grey70")) +
  xlab("") +
  ylab("") +
  labs(subtitle = paste0("p-value = ",p.test))

for(i in 1:length(lab.pos.stages)){
  for(j in 1:length(lab.pos.stages[[i]])){
    p <- p + geom_text(x = i, y = lab.pos.stages[[i]][j], label = paste0(names(lab.pos.stages[[i]])[j]), inherit.aes = F, color = "black", size = 4)
  }
}

# Lehmann TNBCtypes vs pCR

####

df.temp<-df[df$TNBC6!="ND",]
df2<-data.frame(GeneSet="TNBC",
                TNBC=droplevels(df.temp$TNBC6),
                pCR=df.temp$pCR,
                Arm=df.temp$Arm)
df2$Group<-factor(paste(df2$Arm,df2$TNBC),levels=c("CT/A BL1","CT/A BL2","CT/A IM","CT/A LAR","CT/A M","CT/A MSL",
                                                   "CT BL1","CT BL2","CT IM","CT LAR","CT M","CT MSL"))
t2x2<-table(df2$Group,df2$pCR)
perc<-round(t2x2/rowSums(t2x2)*100,1)
t2x2.overall<-table(df2$TNBC,df2$pCR)
perc.overall<-round(t2x2.overall/rowSums(t2x2.overall)*100,1)
...
df.perc<-data.frame(Perc=c(perc[,1],perc.overall[,1]),
                    N=c(t2x2[,1],t2x2.overall[,1]),
                    TNBC=c(gsub(".* ","",rownames(t2x2)),rownames(t2x2.overall)),
                    Arm=c(gsub(" .*","",rownames(t2x2)),rep("CT/A+CT",6)),
                    Group=c(rownames(t2x2),rownames(t2x2.overall)))
df.perc$Arm<-factor(df.perc$Arm, levels=c("CT/A+CT","CT/A","CT"))
df.perc$Group<-factor(df.perc$Group,levels=c("CT/A BL1","CT/A BL2","CT/A IM","CT/A LAR","CT/A M","CT/A MSL",
                                             "CT BL1","CT BL2","CT IM","CT LAR","CT M","CT MSL",
                                             "BL1","BL2","IM","LAR","M","MSL"))
df.perc$TNBC<-factor(df.perc$TNBC, levels=c("LAR","BL1","BL2","IM","M","MSL"))
df.perc<-df.perc[order(df.perc$Arm, df.perc$TNBC),]
df.perc$Position<-c(1:6,8:13,15:20)

p2<-ggplot(data=df.perc, aes(x=Position, y=Perc, fill=TNBC)) +
  geom_bar(stat="identity", width=0.99, position = position_dodge(width=0.9)) +
  theme_pubr(border=F, legend="bottom") +
  xlab("") +
  ylab("pCR rate (%)") +
  scale_x_continuous(breaks=c(3.5,10.5,17.5), labels=c("CT/A+CT","CT/A","CT")) +
  scale_y_continuous(breaks=seq(0,100,20),limits = c(0,100)) +
  geom_text(aes(x=Position,y=Perc+1,label=paste0(round(Perc,0),"%")), color="black", size=4, angle=90, hjust=0) +
  scale_fill_manual(name="",values = c(pal_npg("nrc")(5),"gold")[c(3,1,2,6,4,5)]) +
...
pdf(paste0(outputPath, "/Supplementary_Figure_6.pdf"),width=11,height=5.5)
cowplot::plot_grid(p,p2,ncol=2, labels = c("A","B"), label_size=20)
dev.off()


#********************************************************
# Supplementary Figure 7 - TNBC types vs IO score
#********************************************************
...
pdf(paste0(outputPath, "/Supplementary_Figure_7.pdf"),width=7,height=5)
